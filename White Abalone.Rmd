...
```{r}
WA<- readr::read_csv("kristen2.csv")
```
...
```{r}
##created a new data frame that groups trough with GCS(number of non zero gonad scores) and mean mass (MM) and Mean legnth (ML) Ill use this to make the app ((calculated all of these out by grouping by tough and summarizing by another variable))
Trough<- c(1,2,3,4)
GCS<- c(5,9,7,14)
...
server <- function(input, output, session) { 
  ...
  output$plot <- renderPlot({
    ggplot(APP, aes_string(x = input$x, y =  input$y, fill = input$x)) + geom_col() + theme_light(base_size = 18)
  })
